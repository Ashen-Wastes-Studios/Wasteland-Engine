...
void main() 
{
    ...
    vec4 pixelColor = vec4(0.25, 0.5, 1.0, 1.0);
    ...
    for(int i = 0; i < u_InstanceCount; i++) 
    {
        ...
        if (uint(inst.Properties.x) == 0) tLocal = HitCube(localRay, localNormal);
        else if (uint(inst.Properties.x) == 1) tLocal = HitSphere(localRay, inst.Properties.y, localNormal);
        ...
        if (tLocal > 0.0)
        {
            ...
            if (tWorld < closestHit)
            {
                ...
                // --- SHADOW CALCULATION ---
                float shadow = 1.0;
                vec3 shadowOrigin = worldHitPoint + (worldNormal * 0.005); // Bias
                
                for(int j = 0; j < u_InstanceCount; j++) 
                {
                    RayTracingInstance occluder = Instances[j];
                    Ray localShadowRay;
                    localShadowRay.Origin = (occluder.InvTransform * vec4(shadowOrigin, 1.0)).xyz;
                    localShadowRay.Direction = (occluder.InvTransform * vec4(lightDir, 0.0)).xyz;
                    
                    vec3 dummy;
                    float tShadow = -1.0;
                    if (uint(occluder.Properties.x) == 0) tShadow = HitCube(localShadowRay, dummy);
                    else if (uint(occluder.Properties.x) == 1) tShadow = HitSphere(localShadowRay, occluder.Properties.y, dummy);
                    
                    if (tShadow > 0.0) { shadow = 0.2; break; }
                }
                
                float lightIntensity = max(dot(worldNormal, lightDir), 0.1);
                pixelColor = inst.Color * lightIntensity * shadow;
            }
        }
    }
